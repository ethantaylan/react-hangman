...
const HangmanHard: React.FC = () => {
  ...
  // Hard DIFFICULTY
  const getRandomHardWord = (HardWords: string[]) => {
    const randomIndex = Math.floor(Math.random() * HardWords.length);
    return HardWords[randomIndex];
  };
  const [randomHardWord, setrandomHardWord] = React.useState<string>(
    getRandomHardWord(hardWords).toUpperCase()
  );
  ...
  const resetHardDifficultyGame = () => {
    const randomHardWord = getRandomHardWord(hardWords).toUpperCase();
    setrandomHardWord(randomHardWord);
    setSelectedWord(randomHardWord);
    setRemainingTries(11);
    setCorrectGuesses([""]);
    setNewGameBtn(false);
    setHangman(newgame);
    setAlphabet(defaultAlphabet);
    setReplacedWord(replaceWithUnderscores(randomHardWord));
  };
  ...
  React.useEffect(() => {
    if (replacedWord.join("").includes(randomHardWord)) {
      setWin(true);
      setShow(true);
    }
  }, [replacedWord]);
